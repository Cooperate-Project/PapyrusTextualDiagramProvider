...
class BintrayAPI:
	...
	def __init__(self, username, apikey, repository, package, version):
	...
	def clean(self):
		requestURL = 'https://api.bintray.com/packages/' + self.username + '/' + self.repository + '/' + self.package + '/versions/' + self.version
	...
	def uploadFile(self, relativePath, absolutePath, explode=False):
		requestURL = 'https://api.bintray.com/content/' + self.username + '/' + self.repository + '/' + self.package + '/' + self.version + '/' + self.version + '/' + relativePath
...
def main():
	...
	bintrayAPI = BintrayAPI(args.username, args.apikey, args.repository, args.package, args.version)
	bintrayAPI.clean()
